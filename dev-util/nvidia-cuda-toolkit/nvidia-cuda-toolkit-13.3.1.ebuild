...
cuda-toolkit_check_reqs() {
	if use amd64; then
		export CHECKREQS_DISK_BUILD="7500M"
	elif use arm64; then
		export CHECKREQS_DISK_BUILD="7200M"
	fi
...
src_install() {
	...
	if ! use rdma; then
		...
		rm -f "${ED}/${CUDA_PATH}/targets/${narch}-linux/lib/libcuobjclient"* || die "failed to remove cuobjclient files"
	else
		# cuobjclient is bundled in CUDA 13.1.1+; generate its pkg-config file so
		# that consumers like nixl can find it via dependency('cuobjclient-13.3').
		if [[ -f "${ED}/${CUDA_PATH}/targets/${narch}-linux/lib/libcuobjclient.so" ]]; then
			dodir "${CUDA_PATH}/pkgconfig"
			cat > "${ED}${CUDA_PATH}/pkgconfig/cuobjclient-13.3.pc" <<-EOF || die
				cudaroot=${EPREFIX}${CUDA_PATH}
				libdir=\${cudaroot}/targets/${narch}-linux/lib
				includedir=\${cudaroot}/targets/${narch}-linux/include

				Name: cuobjclient
				Description: NVIDIA cuObject client library
				Version: ${PV}
				Libs: -L\${libdir} -lcuobjclient
				Cflags: -I\${includedir}
			EOF
		else
			ewarn "libcuobjclient.so not found — cuobjclient-13.3.pc not installed"
			ewarn "Check the CUDA manifest for the cuobjclient component name."
		fi
	fi
